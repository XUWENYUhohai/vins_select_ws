...
{
#ifdef UNIT_SPHERE_ERROR//在parameters.h中被注释掉了
...
bool ProjectionFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    Eigen::Vector3d pts_camera_i = pts_i / inv_dep_i;// 地图点在i帧相机坐标系下坐标
    Eigen::Vector3d pts_imu_i = qic * pts_camera_i + tic; // 转成第i帧imu坐标系
    Eigen::Vector3d pts_w = Qi * pts_imu_i + Pi; // 转成世界坐标系
    Eigen::Vector3d pts_imu_j = Qj.inverse() * (pts_w - Pj);// 转到第j帧imu坐标系
    Eigen::Vector3d pts_camera_j = qic.inverse() * (pts_imu_j - tic);   // 转到第j帧相机坐标系
    ...
    if (jacobians)
    {
        ...
        if (jacobians[0])
        {
            ...
            jaco_i.leftCols<3>() = ric.transpose() * Rj.transpose();//对i时刻位移求导
            jaco_i.rightCols<3>() = ric.transpose() * Rj.transpose() * Ri * -Utility::skewSymmetric(pts_imu_i);//对角度求导（李代数右乘）
        ...
        if (jacobians[1])
        {
            ...
            jaco_j.leftCols<3>() = ric.transpose() * -Rj.transpose();//对j时刻位移求导
        ...
        if (jacobians[2])
        {
            ...
            jaco_ex.leftCols<3>() = ric.transpose() * (Rj.transpose() * Ri - Eigen::Matrix3d::Identity());//对外参求导
...
#if 1
            jacobian_feature = reduce * ric.transpose() * Rj.transpose() * Ri * ric * pts_i * -1.0 / (inv_dep_i * inv_dep_i);//对逆深度求导
#else
