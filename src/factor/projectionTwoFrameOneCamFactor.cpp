 ...
 * @param _pts_i        前一帧归一化相机点         （其实都是每个特征点的首帧）
 * @param _pts_j        当前帧归一化相机点
 * @param _velocity_i   前一帧速度（归一化相机平面）（其实都是每个特征点的首帧）
 * @param _velocity_j   当前帧速度（归一化相机平面）
 * @param _td_i         前一帧相机与IMU时差       （其实都是每个特征点的首帧）
...
#ifdef UNIT_SPHERE_ERROR// 采用另一种误差模型，公式意义参考原论文
...
bool ProjectionTwoFrameOneCamFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    Eigen::Vector3d pts_camera_i = pts_i_td / inv_dep_i;// 转到相机系
    Eigen::Vector3d pts_imu_i = qic * pts_camera_i + tic;// 转到IMU系
    Eigen::Vector3d pts_w = Qi * pts_imu_i + Pi;  // 转到世界系
    Eigen::Vector3d pts_imu_j = Qj.inverse() * (pts_w - Pj);// 转到j的IMU系
    Eigen::Vector3d pts_camera_j = qic.inverse() * (pts_imu_j - tic); // 转到j的相机系
    ...
    // residual = 0.00015 * sqrt_info * residual;//todo
    residual = sqrt_info * residual;//todo     三个loss_function: 0.000001  100000  10000000(应该更大也行)
