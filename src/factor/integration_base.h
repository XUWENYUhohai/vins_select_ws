...
{
  public:
    IntegrationBase() = delete;//禁止生成默认构造函数
    ...
    {   //这个noise是预积分的噪声方差矩阵
        ...
        noise.block<3, 3>(0, 0) =  (ACC_N * ACC_N) * Eigen::Matrix3d::Identity();// 中值积分，噪声项有6项；欧拉积分，噪声项为4项（对应vins-mono论文公式9的nt）
    ...
    */
    void push_back(double dt, const Eigen::Vector3d &acc, const Eigen::Vector3d &gyr)
    {
        ...
        propagate(dt, acc, gyr);//进行预积分和传播
    }

    // 再传播函数 根据新的bias重新计算预积分,状态清零，设置新偏置，中值积分传播
    void repropagate(const Eigen::Vector3d &_linearized_ba, const Eigen::Vector3d &_linearized_bg)
    {  //再次进行预积分重传播，通常是由于imu的bias发生较大的改变，预积分值不能通过雅克比进行更新，需要重新预积分传播进行更新
        sum_dt = 0.0;
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        result_linearized_ba = linearized_ba;//两图像帧之间，IMU bias保持固定
        result_linearized_bg = linearized_bg;         

        if(update_jacobian)//根据传入参数，决定是否更新雅克比矩阵
        {
            ...
            MatrixXd F = MatrixXd::Zero(15, 15);//雅克比，// 当前时刻误差对前一时刻误差的微分
            ...
            MatrixXd V = MatrixXd::Zero(15,18);//协方差G，// 当前误差对噪声的微分
 ...
 /**
    ...
    */
    ...
    // 预积分结束时刻误差相对于预积分起始时刻误差的微分
        ...
        Eigen::Quaterniond corrected_delta_q = delta_q * Utility::deltaQ(dq_dbg * dbg);//带噪声修正
        Eigen::Vector3d corrected_delta_v = delta_v + dv_dba * dba + dv_dbg * dbg;//线性近似，预积分的调整：零偏仍在积分项中
